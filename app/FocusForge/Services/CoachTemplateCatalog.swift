...
// MARK: - Template Condition

enum TemplateCondition: String {
    case `default`
    case lowCompletion   // completionRate7d < 0.5
    case highAbandon     // abandonRate7d > 0.3
    case newUser         // totalSessions7d < 3
    case highCompletion  // completionRate7d >= 0.8
    case longSession     // actualMinutes >= 40
    case shortSession    // actualMinutes < 15
}

// MARK: - Framing Template

struct FramingTemplate {
    let id: String
    let category: String
    let condition: TemplateCondition
    let reframeFormat: [CoachTone: String]        // %@ = task name
    let motivationalLine: [CoachTone: String]
}

// MARK: - Reflection Template
...
struct ReflectionTemplate {
    let id: String
    let condition: TemplateCondition
    let category: ReflectionCategory
    let tipText: [CoachTone: String]
}
...
struct NudgeTemplate {
    let id: String
    let streakTier: ClosedRange<Int>   // e.g. 1...3, 4...7
    let title: [CoachTone: String]
    let body: [CoachTone: String]      // %d = streak days
}

// MARK: - Catalog

enum CoachTemplateCatalog {
    ...
    // MARK: - Framing Templates (~15)

    static let framingTemplates: [FramingTemplate] = [
        // General — default
        FramingTemplate(
    ...
    // MARK: - Reflection Templates (~10)

    static let reflectionTemplates: [ReflectionTemplate] = [
        // Default completed
        ReflectionTemplate(
        ...
        // High abandon — but they completed this one
        ReflectionTemplate(
        ...
        // High completion — stretch suggestion
        ReflectionTemplate(
    ...
    // MARK: - Nudge Templates (~8)

    static let nudgeTemplates: [NudgeTemplate] = [
